...
jQuery.imgAreaSelect.init = function (img, options) {
    var $area = jQuery('<div></div>'),
        ...
        left, top, imgOfs, imgWidth, imgHeight, parent, parOfs, parScroll, adjusted, zIndex = 0,
        fixed, $p, startX, startY, moveX, moveY, resizeMargin = 10,
        resize = [],
        V = 0,
        H = 1,
        keyDown, d, aspectRatio, x1, x2, y1, y2, x, y, selection = {
            x1: 0,
            y1: 0,
            x2: 0,
            y2: 0,
    ...
    function doResize(newX2, newY2) {
        ...
        if (aspectRatio)
            if (Math.abs(x2 - x1) / aspectRatio > Math.abs(y2 - y1)) aspectRatioYX();
            else aspectRatioXY();
    ...
    var docKeyPress = function (event) {
        var k = options.keys,
            d = 10,
            t, key = event.keyCode || event.which;
    ...
    options.onSelectChange(img, selection); //SherZa
};
